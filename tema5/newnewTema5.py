...
# MAIN PROGRAM - part (b)
Nlist = [10, 50, 100, 200, 500]     # range for matrix sizes
...
# load and plot results from Fortran-performance test
tmp = np.loadtxt('exercise8_Fortrantimes.dat')
plt.loglog(tmp[:,0],tmp[:,2],'r^:' ,label='LAPACK (dsyev) [Fortran]')
plt.loglog(tmp[:,0],tmp[:,1],'bo:' ,label='Jacobi [Fortran]')
